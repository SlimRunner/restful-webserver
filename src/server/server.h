...
using SessionFactory = std::function<session *(boost::asio::io_service &, std::vector<std::shared_ptr<RequestHandler>>)>;

class server
{
public:
  // Refactored the constructor so that we have a default session unless we give a mock session
  server(boost::asio::io_service &io_service, short port, std::vector<std::shared_ptr<RequestHandler>> handlers, SessionFactory factory = [](auto &io, auto handlers)
