...
protected:
  ...
  void SetUp() override {
    ...
    ON_CALL(*mock_handler_, CanHandle(testing::_))
        .WillByDefault(testing::Invoke(
            [](const std::string &path) { return path == "/echo"; }));
  ...
  void injectData(const std::string &req) { session_->request_buffer_ = req; }
...
TEST_F(SessionTest, SessionStartsCleanly) { session_->start(); }
...
TEST_F(SessionTest, SessionDirectSetRequest) {
  ...
  std::string fake_request = "GET / HTTP/1.1\r\n"
                             "Host: localhost\r\n"
                             "User-Agent: test-client\r\n"
                             "\r\n";
...
TEST_F(SessionTest, HandleReadWithOversize) {
  ...
  EXPECT_FALSE(session_->already_deleted_);
  constexpr auto expectedResp = "HTTP/1.1 413 Payload Too Large\r\n"
                                "Content-Length: 0\r\n"
                                "Connection: close\r\n"
                                "\r\n";
...
TEST_F(SessionTest, HandleReadCompleteRequest) {
  std::string fullReq = "GET /echo HTTP/1.1\r\n"
                        "Host: localhost\r\n"
                        "Connection: close\r\n\r\n";
  boost::system::error_code successCode{};
...
TEST_F(SessionTest, HandleReadUnhandledRequest) {
  std::string fullReq = "GET /mockfails HTTP/1.1\r\n"
                        "Host: localhost\r\n\r\n";
  boost::system::error_code successCode{};
  ...
  session_->handle_read(successCode, fullReq.size());
  EXPECT_FALSE(session_->already_deleted_);
  EXPECT_NE(session_->current_response_.find("404 Not Found"),
            std::string::npos);
}
...
TEST_F(SessionTest, HandleBadVersionRequest) {
  std::string fullReq = "GET /echo HTTP/2.0\r\n"
                        "Host: localhost\r\n"
                        "Connection: close\r\n"
                        "\r\n";
  boost::system::error_code successCode{};
  const std::string actualRes = "HTTP/1.1 400 Bad Request\r\n"
                                "Content-Length: 0\r\n"
