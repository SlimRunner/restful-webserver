...
TEST_F(ServerTest, UsesInjectedSessionFactory) {
  ...
  SessionFactory test_factory = [&](auto& io_ref) {
    ++factory_call_count;
    return new session(io_ref);
  };

  // Create server using custom factory
  server test_server(io, 8080, test_factory);
...
TEST_F(ServerTest, HandleAcceptCallsStart) {
  ...
  TestableServer test_server(io, 8080, [](auto& io_ref) {
    return new session(io_ref);  // return a dummy
  });
...
TEST_F(ServerTest, HandleAcceptOnErrorDeletesSession) {
    ...
    TestableServer test_server(io, 8080, [](auto& io_ref) {
    return new session(io_ref);  // dummy factory
    });
...
TEST_F(ServerTest, StartAcceptCreatesNewSession) {
  ...
  TestableServer test_server(io, 8080, test_factory);
