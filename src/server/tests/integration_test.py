...
def createPOSTRequest(host, name):
    return (
        "\r\n".join(
            [f"POST /{name} HTTP/1.1", f"Host: {host}", "Content-Length: 0", "", ""]
        )
    ).encode()
...
def run_server():
    ...
    with open(error_log, "w") as err_file:
        server_process = subprocess.Popen(
            [SERVER_BINARY, TEST_CONFIG], stderr=err_file, stdout=subprocess.PIPE
        )
    ...
    if server_process and server_process.poll() is not None:
        ...
        if exit_code < 0:
            signal_name = (
                ...
                if -exit_code in signal.Signals
                else "UNKNOWN"
            )
...
def send_request_and_check(
    test_name, request_data, expected_status, save_response=True
):
    ...
    try:
        ...
        if response_text.startswith(expected_status):
            print(
                f"Test {test_name}: PASSED - Response contains expected status: {expected_status}"
            )
            return True
        else:
            print(
                f"Test {test_name}: FAILED - Response doesn't contain expected status: {expected_status}"
            )
...
def test_multiple_requests_same_connection():
    ...
    try:
        ...
        # Save first response
        with open(
            os.path.join(TEST_OUTPUT_DIR, "multiple_requests_first.txt"), "w"
        ) as f:
        ...
        if first_request_passed:
            print(
                "First request: PASSED - Response contains expected status: HTTP/1.1 200 OK"
            )
        else:
            print(
                "First request: FAILED - Response doesn't contain expected status: HTTP/1.1 200 OK"
            )
        ...
        with open(
            os.path.join(TEST_OUTPUT_DIR, "multiple_requests_second.txt"), "w"
        ) as f:
        ...
        if second_request_passed:
            print(
                "Second request: PASSED - Response contains expected status: HTTP/1.1 200 OK"
            )
        else:
            print(
                "Second request: FAILED - Response doesn't contain expected status: HTTP/1.1 200 OK"
            )
...
def run_all_tests():
    ...
    test_results["multiple_requests_same_connection"] = (
        test_multiple_requests_same_connection()
    )
